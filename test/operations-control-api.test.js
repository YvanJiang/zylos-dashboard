...
async function withServer(client, run) {
  ...
  const api = createOperationsControlApi({ authorizationAdapter: adapter, client, resultStore: new OperationsControlResultStore(), getVerifiedSubject: (req) => req.headers['x-test-subject'] ? subject : null, readJsonBody, sendJson });
